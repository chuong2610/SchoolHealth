...
const Login = () => {
  ...
  const checkPhoneVerification = async (phoneNumber) => {
    try {
      const response = await axios.get(
        `http://localhost:5182/api/auth/is-verified/${phoneNumber}`
      );
  ...
  const sendOTP = async (phoneNumber) => {
    try {
      const response = await axios.post(
        "http://localhost:5182/api/auth/send-otp",
        {
  ...
  const verifyOTP = async (phoneNumber, otpCode) => {
    try {
      const response = await axios.post(
        "http://localhost:5182/api/auth/verify-otp",
        {
          phoneNumber: phoneNumber,
          otp: otpCode,
        }
  ...
  const updatePassword = async (phoneNumber, newPassword) => {
    try {
      const response = await axios.post(
        "http://localhost:5182/api/User/update-password",
        {
          phoneNumber: phoneNumber,
  ...
  const handleOTPSubmit = async (e) => {
    e.preventDefault();
    if (!otp) {
      setError("Vui lòng kiểm tra Mail và nhập mã OTP!");
      return;
    }
    setLoading(true);
    setError("");
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5182/api/auth/login",
        {
          phoneNumber: phoneNumber,
          password: password,
        }
  ...
  return (
    ...
    >
      <div
        className="container"
        style={{ maxWidth: 620, maxHeight: 800, zIndex: 2 }}
      >
        <div className="text-center mb-3">
          <i
            className="fas fa-heartbeat"
            style={{ color: "#2563eb", fontSize: 40 }}
          ></i>
