...
function AuthCallback() {
    ...
    useEffect(() => {
        const handleGoogleCallback = async () => {
            ...
            try {
                ...
                setLoginDone(true); // Đánh dấu login xong, chờ context cập nhật user
    ...
    // Theo dõi user, chỉ redirect khi loginDone và user đã cập nhật
    useEffect(() => {
        if (loginDone && user && user.role) {
            const targetPath = `/${user.role.toLowerCase()}`;
            navigate(targetPath, { replace: true });
        }
    }, [loginDone, user, navigate]);
